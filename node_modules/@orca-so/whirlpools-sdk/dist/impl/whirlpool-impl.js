...
class WhirlpoolImpl {
    ...
    async openPosition(tickLower, tickUpper, liquidityInput, wallet, funder, positionMint, tokenProgramId, resolveATA) {
        await this.refresh();
        return this.getOpenPositionWithOptMetadataTx(tickLower, tickUpper, liquidityInput, !!wallet ? common_sdk_1.AddressUtil.toPubKey(wallet) : this.ctx.wallet.publicKey, !!funder ? common_sdk_1.AddressUtil.toPubKey(funder) : this.ctx.wallet.publicKey, tokenProgramId ?? spl_token_1.TOKEN_PROGRAM_ID, false, positionMint, resolveATA);
    }
    async openPositionWithMetadata(tickLower, tickUpper, liquidityInput, sourceWallet, funder, positionMint, tokenProgramId, resolveATA) {
        await this.refresh();
        return this.getOpenPositionWithOptMetadataTx(tickLower, tickUpper, liquidityInput, !!sourceWallet
            ...
            : this.ctx.wallet.publicKey, !!funder ? common_sdk_1.AddressUtil.toPubKey(funder) : this.ctx.wallet.publicKey, tokenProgramId ?? spl_token_1.TOKEN_PROGRAM_ID, true, positionMint, resolveATA);
    }
    ...
    async getOpenPositionWithOptMetadataTx(tickLower, tickUpper, liquidityInput, wallet, funder, tokenProgramId, withMetadata = false, positionMint, resolveATA = true) {
        ...
        if (resolveATA || whirlpool.tokenMintA.equals(spl_token_1.NATIVE_MINT)) {
            txBuilder.addInstruction(tokenOwnerAccountAIx);
        }
        if (resolveATA || whirlpool.tokenMintB.equals(spl_token_1.NATIVE_MINT)) {
