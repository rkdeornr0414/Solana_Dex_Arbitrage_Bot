...
function simulateSwap(params) {
    const { aToB, whirlpoolData, tickArrays, tokenAmount, sqrtPriceLimit, otherAmountThreshold, amountSpecifiedIsInput, timestampInSeconds: optionalTimestampInSeconds, oracleData, tokenExtensionCtx, } = params;
    ...
    if ((aToB && sqrtPriceLimit.gte(whirlpoolData.sqrtPrice)) ||
    ...
    const adaptiveFeeInfo = !!oracleData
        ...
        : null;
    const timestampInSeconds = optionalTimestampInSeconds ?? new anchor_1.BN(Date.now()).div(new anchor_1.BN(1000));
    if (oracleData?.tradeEnableTimestamp.gt(timestampInSeconds)) {
        throw new errors_1.WhirlpoolsError("Trade is not enabled yet.", errors_1.SwapErrorCode.TradeIsNotEnabled);
    }
    if (amountSpecifiedIsInput) {
        ...
        const swapResults = (0, swap_manager_1.computeSwap)(whirlpoolData, tickSequence, transferFeeExcludedIn.amount, sqrtPriceLimit, amountSpecifiedIsInput, aToB, timestampInSeconds, adaptiveFeeInfo);
    ...
    const transferFeeIncludedOut = token_extension_util_1.TokenExtensionUtil.calculateTransferFeeIncludedAmount(tokenAmount, aToB
        ? tokenExtensionCtx.tokenMintWithProgramB
        : tokenExtensionCtx.tokenMintWithProgramA, tokenExtensionCtx.currentEpoch);
    const swapResults = (0, swap_manager_1.computeSwap)(whirlpoolData, tickSequence, transferFeeIncludedOut.amount, sqrtPriceLimit, amountSpecifiedIsInput, aToB, timestampInSeconds, adaptiveFeeInfo);
